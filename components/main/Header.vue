			...
			<div id="mobileNav" class="nav-menu mobile-menu-closed">
				<div class="mobile">
					<div v-if="user.connected" class="link-container">
						...
						<a
							class="u-animation-2"
							href="https://api.mle-moni.fr/logout?redir=http://notes.local.fr/"
						>
							Se déconnecter
						</a>
					</div>
					<div v-else class="link-container">
						<a
							class="u-animation-2"
							href="https://api.mle-moni.fr/login?redir=http://notes.local.fr/"
						>
							Se connecter
						</a>
						<a
							class="u-animation-2"
							href="https://api.mle-moni.fr/register?redir=http://notes.local.fr/"
						>
							Créer un compte
						</a>
					</div>
...
export default {
	...
	async fetch() {
		this.user = await fetch('https://api.mle-moni.fr/user-info', {
			credentials: 'include',
		}).then((res) => res.json())
